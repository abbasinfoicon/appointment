...
const Appointment = () => {
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                Cell: ({ row }) => (
                    <div>
                        <Link href={`/dashboard/doctors/${docId.id}/prescription/?appId=${row.original.id}`} className={`btn rounded btn-success ${isDatePassed(row.original.slot_date) ? 'disabled' : ''}`} title='Prescription'><i className="fa fa-medkit"></i></Link>
                        <Link href={`/dashboard/appointments/${row.original.id}?docId=${docId.id}`} className='btn rounded btn-primary mx-1' title='View'><i className="icon-eye"></i></Link>
                        <Link href={`/dashboard/appointments/edit/${row.original.id}?docId=${docId.id}`} title='Edit' className={`btn rounded btn-info mx-1 ${isDatePassed(row.original.slot_date) ? 'disabled' : ''}`}><i className="icon-pencil"></i></Link>
    ...
    }, [token, search, deleteContent, docId]);
    ...
    return (
        <div className="container-fluid">
                    ...
                    <div className="card">
                        <div className="card-header">
                            ...
                            <div className="add-new text-right form-filter-date">
                                ...
                                <Link href={`/dashboard/appointments/add?dId=${docId.id}`} className="btn btn-outline-primary btn-lg btn-rounded mt-1 pl-5 pr-5 add-new">Add New</Link>
                            </div>
                        </div>
            ...
            <DeleteModal did={deleteId} dc={deleteContent} setdc={setDeleteContent} page={"doctors/" + docId.id + "/appointment"} url="app/deleteAppointment" />
        </div>
    )
}
